# Start: powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\PurplePOC-1.0.11-gui-black-console-header.ps1
param(
...
Write-Host "v1.0.11" -NoNewline -ForegroundColor Magenta
...
- id: exfil_http_loopback
  ...
  execution_details:
    ...
    network: NONE — no HTTP request or POST is sent in v1.0.11
...
<div class="footer">PurplePOC v1.0.11 &nbsp;•&nbsp; Detection Validation Report</div>
</div></body></html>
"""
...
def test_version_is_consistent():
    ...
    assert 'else "1.0.11"' in rich
    assert "v1.0.11" in ui
...
if (
    ...
    if ($NeedsModule) {
        if (-not (Test-InternetName -Name "www.powershellgallery.com")) {
    ...
    if ($Force -or $NeedsDefinitions) {
        if (-not (Test-InternetName -Name "raw.githubusercontent.com")) {
        ...
        $InstallerResponse = Invoke-WebRequest `
            -Uri $InstallerUrl `
            -UseBasicParsing `
            -TimeoutSec 30 `
            -ErrorAction Stop
...
& $Python -m pip install --disable-pip-version-check --upgrade certipy-ad
...
if(-not (Test-Path $Python)){& "$Root\Bootstrap.ps1";if(Test-Path $RuntimeFile){$Python=(Get-Content $RuntimeFile -Raw).Trim()}}
$env:PURPLEPOC_PYTHON=$Python
...
function Show-PurplePOCHelp {
    Write-Host ""
    Write-Host "PurplePOC 1.0.11 - Selective MITRE Execution" -ForegroundColor Cyan
    Write-Host ""
...
PurplePOC 1.0.11 adds AUTO coverage for Windows event-log manipulation and
...
PurplePOC 1.0.11 expands the AUTO/Atomic scenario substantially:
...
PurplePOC 1.0.11 fixes a false-negative smoke test introduced in 1.9.4.
...
PurplePOC 1.0.11 fixes scenario startup exiting with code 2 before T-001.
...
PurplePOC 1.0.11 removes stale smoke-test assertions left behind by the
...
The smoke test now validates the actual 1.0.11 parser implementation and keeps
...
PurplePOC 1.0.11 fixes the generated `rich_app.py` indentation error in the
...
PurplePOC 1.0.11 fixes the Rich UI crash caused by referencing `args.guided`
...
PurplePOC 1.0.11 is unattended by default: AUTO/Atomic tests run normally,
...
PurplePOC 1.0.11 replaces the large raw-output area in the live Rich UI with a
...
PurplePOC 1.0.11 fixes the dashboard appearing idle while scenario work was
...
PurplePOC 1.0.11 changes the Rich Live renderer for Windows PowerShell/conhost.
...
PurplePOC 1.0.11 fixes the behavior where the Rich UI disappeared as soon as
...
PurplePOC 1.0.11 fixes the Rich UI abort with exit code `5`.
...
PurplePOC 1.0.11 fixes the actual generator bug behind the missing
`Setup-AtomicRedTeam.ps1`.
...
PurplePOC 1.0.11 synchronizes the displayed version with the generated
...
PurplePOC 1.0.11 uses a real Python Rich terminal application for the elevated
...
PurplePOC 1.0.11 replaces the old verbose scenario output with a compact
structured renderer.
...
PurplePOC 1.0.11 gives the elevated PowerShell child process the same branded
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\PurplePOC-1.0.11-execution-bypass-header.ps1
```
...
## Interactive MITRE dashboard (v1.0.11)
