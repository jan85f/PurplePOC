# Start: powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\PurplePOC-1.0.13-proxy-complete.ps1
param(
...
Write-Host "v1.0.13" -NoNewline -ForegroundColor Magenta
...
- id: exfil_http_loopback
  ...
  execution_details:
    ...
    network: NONE — no HTTP request or POST is sent in v1.0.13
...
<div class="footer">PurplePOC v1.0.13 &nbsp;•&nbsp; Detection Validation Report</div>
</div></body></html>
"""
...
def test_version_is_consistent():
    ...
    assert 'else "1.0.13"' in rich
    assert "v1.0.13" in ui
...
$Targets = @(
    ...
    @{
        Name = "GitHub Raw"
        Uri = "https://raw.githubusercontent.com/redcanaryco/invoke-atomicredteam/master/install-atomicsfolder.ps1"
    },
    @{
    ...
    try {
        ...
        try {
            ...
            if ($Target.Name -eq "Atomic Red Team archive") {
                ...
                $Args.Uri = "https://github.com/redcanaryco/atomic-red-team"
            }
...
if (
    ...
    if ($Force -or $NeedsDefinitions) {
        ...
        $ArchiveUrl = "https://github.com/redcanaryco/atomic-red-team/archive/refs/heads/master.zip"
        $ArchiveFile = Join-Path $AtomicBase "atomic-red-team-master.zip"
        $ExtractRoot = Join-Path $AtomicBase "_purplepoc_download"

        Remove-Item $ArchiveFile -Force -ErrorAction SilentlyContinue
        Remove-Item $ExtractRoot -Recurse -Force -ErrorAction SilentlyContinue

        Write-Step "Downloading official Atomic Red Team definitions"
        Write-Info "Network source" $ArchiveUrl
        Write-Info "Install path" $AtomicDefinitions
        if (-not [string]::IsNullOrWhiteSpace($Proxy)) {
            Write-Info "Transport" "EXPLICIT PROXY"
        }
        else {
            Write-Info "Transport" "DIRECT / system default"
        }
        ...
        if (-not (Test-Path $ArchiveFile)) {
            throw "Atomic Red Team archive download did not create $ArchiveFile"
        }

        New-Item -ItemType Directory -Path $ExtractRoot -Force | Out-Null
        Expand-Archive -LiteralPath $ArchiveFile -DestinationPath $ExtractRoot -Force

        $ExtractedAtomics = Join-Path $ExtractRoot "atomic-red-team-master\atomics"

        if (-not (Test-Path $ExtractedAtomics)) {
            throw "Downloaded Atomic archive does not contain the expected atomics folder."
        }
        ...
        Move-Item -LiteralPath $ExtractedAtomics -Destination $AtomicDefinitions -Force

        Remove-Item $ExtractRoot -Recurse -Force -ErrorAction SilentlyContinue
        Remove-Item $ArchiveFile -Force -ErrorAction SilentlyContinue

        Write-Pass "Atomic definitions installed through PurplePOC proxy-aware downloader"
...
function Show-PurplePOCHelp {
    Write-Host ""
    Write-Host "PurplePOC 1.0.13 - Selective MITRE Execution" -ForegroundColor Cyan
    Write-Host ""
...
PurplePOC 1.0.13 adds AUTO coverage for Windows event-log manipulation and
...
PurplePOC 1.0.13 expands the AUTO/Atomic scenario substantially:
...
PurplePOC 1.0.13 fixes a false-negative smoke test introduced in 1.9.4.
...
PurplePOC 1.0.13 fixes scenario startup exiting with code 2 before T-001.
...
PurplePOC 1.0.13 removes stale smoke-test assertions left behind by the
...
The smoke test now validates the actual 1.0.13 parser implementation and keeps
...
PurplePOC 1.0.13 fixes the generated `rich_app.py` indentation error in the
...
PurplePOC 1.0.13 fixes the Rich UI crash caused by referencing `args.guided`
...
PurplePOC 1.0.13 is unattended by default: AUTO/Atomic tests run normally,
...
PurplePOC 1.0.13 replaces the large raw-output area in the live Rich UI with a
...
PurplePOC 1.0.13 fixes the dashboard appearing idle while scenario work was
...
PurplePOC 1.0.13 changes the Rich Live renderer for Windows PowerShell/conhost.
...
PurplePOC 1.0.13 fixes the behavior where the Rich UI disappeared as soon as
...
PurplePOC 1.0.13 fixes the Rich UI abort with exit code `5`.
...
PurplePOC 1.0.13 fixes the actual generator bug behind the missing
`Setup-AtomicRedTeam.ps1`.
...
PurplePOC 1.0.13 synchronizes the displayed version with the generated
...
PurplePOC 1.0.13 uses a real Python Rich terminal application for the elevated
...
PurplePOC 1.0.13 replaces the old verbose scenario output with a compact
structured renderer.
...
PurplePOC 1.0.13 gives the elevated PowerShell child process the same branded
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\PurplePOC-1.0.13-execution-bypass-header.ps1
```
...
## Interactive MITRE dashboard (v1.0.13)
...
## Complete proxy routing (v1.0.13)

v1.0.13 closes two proxy gaps from v1.0.12:
