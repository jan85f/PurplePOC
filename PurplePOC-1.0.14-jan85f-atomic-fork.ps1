# Start: powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\PurplePOC-1.0.14-jan85f-atomic-fork.ps1
param(
    ...
    # Atomic Red Team source.
    [string]$AtomicRepoOwner = "jan85f",
    [string]$AtomicRepoName = "atomic-red-team",
    [string]$AtomicBranch = "master"
)
...
Write-Host "v1.0.14" -NoNewline -ForegroundColor Magenta
...
$AtomicSourceFile = Join-Path $Root "data\atomic-source.json"
[ordered]@{
    owner = $AtomicRepoOwner
    repo = $AtomicRepoName
    branch = $AtomicBranch
} | ConvertTo-Json | Set-Content -Path $AtomicSourceFile -Encoding UTF8

Write-Host "[+] " -NoNewline -ForegroundColor Cyan
Write-Host "Atomic source: " -NoNewline -ForegroundColor White
Write-Host ("github.com/{0}/{1} [{2}]" -f $AtomicRepoOwner,$AtomicRepoName,$AtomicBranch) -ForegroundColor DarkCyan
...
- id: exfil_http_loopback
  ...
  execution_details:
    ...
    network: NONE — no HTTP request or POST is sent in v1.0.14
...
def proxy_url():
    """Return the persisted PurplePOC proxy, if configured."""
    path = ROOT / "data" / "proxy.txt"
...
def proxy_preamble():
    """
    Apply the PurplePOC proxy inside Atomic child PowerShell sessions.

    This covers PowerShell web cmdlets, Python/pip, curl/git-style tools
    honoring standard proxy environment variables, and BITS cmdlet calls.
    """
    proxy = proxy_url()
    ...
    return f"""$PurplePOCProxy = '{p}'
$env:HTTP_PROXY = $PurplePOCProxy
$env:HTTPS_PROXY = $PurplePOCProxy
$env:ALL_PROXY = $PurplePOCProxy
$env:PIP_PROXY = $PurplePOCProxy
try {{
    [System.Net.WebRequest]::DefaultWebProxy = New-Object System.Net.WebProxy($PurplePOCProxy)
}} catch {{}}
$PSDefaultParameterValues['Invoke-WebRequest:Proxy'] = $PurplePOCProxy
$PSDefaultParameterValues['Invoke-RestMethod:Proxy'] = $PurplePOCProxy
if (Get-Command Start-BitsTransfer -ErrorAction SilentlyContinue) {{
    $PSDefaultParameterValues['Start-BitsTransfer:ProxyUsage'] = 'Override'
    $PSDefaultParameterValues['Start-BitsTransfer:ProxyList'] = $PurplePOCProxy
}}
"""
...
<div class="footer">PurplePOC v1.0.14 &nbsp;•&nbsp; Detection Validation Report</div>
</div></body></html>
"""
...
def test_version_is_consistent():
    ...
    assert 'else "1.0.14"' in rich
    assert "v1.0.14" in ui
...
def test_v1013_atomic_definitions_use_purplepoc_explicit_proxy_downloader():
    atomic=(ROOT/"Setup-AtomicRedTeam.ps1").read_text(encoding="utf-8")
    assert "atomic-red-team/archive/refs/heads/master.zip" in atomic
    assert '$AtomicDownloadArgs["Proxy"] = $Proxy' in atomic
    assert "Invoke-WebRequest @AtomicDownloadArgs" in atomic
    assert "Install-AtomicsFolder `" not in atomic
    assert "Loading official Atomic Red Team atomics-folder installer" not in atomic

def test_v1013_atomic_prereqs_receive_proxy():
    adapter=(ROOT/"adapters"/"atomic.py").read_text(encoding="utf-8")
    for token in [
        'env["HTTP_PROXY"] = proxy',
        'env["HTTPS_PROXY"] = proxy',
        'env["ALL_PROXY"] = proxy',
        "$PSDefaultParameterValues['Invoke-WebRequest:Proxy']",
        "$PSDefaultParameterValues['Invoke-RestMethod:Proxy']",
        "$PSDefaultParameterValues['Start-BitsTransfer:ProxyList']",
        "script = proxy_preamble() + script",
    ]:
        assert token in adapter

def test_v1013_proxy_diagnostic_exists():
    proxy_test=(ROOT/"Test-PurplePOCProxy.ps1").read_text(encoding="utf-8")
    for token in [
        "pypi.org/simple/",
        "powershellgallery.com/api/v2/",
        "raw.githubusercontent.com",
        "github.com/redcanaryco/atomic-red-team",
        "Proxy = $Proxy",
    ]:
        assert token in proxy_test


def test_v1014_atomic_source_is_jan85f_fork():
    setup=(ROOT/"Setup-AtomicRedTeam.ps1").read_text(encoding="utf-8")
    assert '$AtomicRepoOwner = "jan85f"' in setup
    assert '$AtomicRepoName = "atomic-red-team"' in setup
    assert '$AtomicBranch = "master"' in setup
    assert "git sparse clone" in setup.lower()
    assert '$AtomicArchiveUrl' in setup

def test_v1014_atomic_payload_urls_are_rewritten():
    setup=(ROOT/"Setup-AtomicRedTeam.ps1").read_text(encoding="utf-8")
    assert '"redcanaryco/atomic-red-team"' in setup
    assert '"$AtomicRepoOwner/$AtomicRepoName"' in setup
    assert "upstream redcanaryco references remain in YAML" in setup
    assert "purplepoc-source.json" in setup
...
Write-File "Test-PurplePOCProxy.ps1" @'
[CmdletBinding()]
param(
    [string]$Proxy,
    [string]$AtomicRepoOwner = "jan85f",
...
if ([string]::IsNullOrWhiteSpace($Proxy)) {
    throw "No proxy configured. Supply -Proxy or configure data\proxy.txt."
}
...
Write-Section "PurplePOC Proxy Verification" Cyan
...
$Targets = @(
    ...
    @{
        Name = "GitHub Raw"
        Uri = "https://raw.githubusercontent.com/$AtomicRepoOwner/$AtomicRepoName/$AtomicBranch/README.md"
    },
    @{
        Name = "Atomic Red Team archive"
        Uri = "https://github.com/$AtomicRepoOwner/$AtomicRepoName/archive/refs/heads/$AtomicBranch.zip"
    }
)
...
foreach ($Target in $Targets) {
    Write-Step ("Testing " + $Target.Name)
    ...
    try {
        ...
        Write-Pass ($Target.Name + " reachable through explicit proxy")
        Write-Info "HTTP" ([string]$Response.StatusCode)
    }
    catch {
        # Some endpoints/proxies reject HEAD while GET works. Retry a tiny GET.
        try {
            $Args = @{
                Uri = $Target.Uri
                Proxy = $Proxy
            ...
            if ($Target.Name -eq "Atomic Red Team archive") {
                # Avoid downloading the full archive during diagnostics.
                $Args.Uri = "https://github.com/$AtomicRepoOwner/$AtomicRepoName"
            }
            ...
            $Failures++
            Write-Fail ($Target.Name + " proxy test failed")
...
if ($Failures -gt 0) {
    Write-Fail "$Failures proxy endpoint test(s) failed."
    exit 1
}

Write-Pass "All PurplePOC bootstrap download endpoints are reachable through the configured proxy."
exit 0
'@

Confirm-GeneratedFile "Test-PurplePOCProxy.ps1" -Critical | Out-Null
...
param(
    [switch]$Force,
    [string]$Proxy,
    [string]$AtomicRepoOwner,
    [string]$AtomicRepoName,
    [string]$AtomicBranch
)
...
$AtomicSourceFile = Join-Path $PurplePOCRoot "data\atomic-source.json"
if (Test-Path $AtomicSourceFile) {
    try {
        $SavedAtomicSource = Get-Content $AtomicSourceFile -Raw | ConvertFrom-Json
        if ([string]::IsNullOrWhiteSpace($AtomicRepoOwner)) { $AtomicRepoOwner = [string]$SavedAtomicSource.owner }
        if ([string]::IsNullOrWhiteSpace($AtomicRepoName)) { $AtomicRepoName = [string]$SavedAtomicSource.repo }
        if ([string]::IsNullOrWhiteSpace($AtomicBranch)) { $AtomicBranch = [string]$SavedAtomicSource.branch }
    } catch {}
}
if ([string]::IsNullOrWhiteSpace($AtomicRepoOwner)) { $AtomicRepoOwner = "jan85f" }
if ([string]::IsNullOrWhiteSpace($AtomicRepoName)) { $AtomicRepoName = "atomic-red-team" }
if ([string]::IsNullOrWhiteSpace($AtomicBranch)) { $AtomicBranch = "master" }

$AtomicRepoWeb = "https://github.com/$AtomicRepoOwner/$AtomicRepoName"
$AtomicRepoGit = "$AtomicRepoWeb.git"
$AtomicArchiveUrl = "$AtomicRepoWeb/archive/refs/heads/$AtomicBranch.zip"

Write-Info "Atomic repository" $AtomicRepoWeb
Write-Info "Atomic branch" $AtomicBranch
...
$AtomicSourceMarker = Join-Path $AtomicBase "purplepoc-source.json"
...
$SourceMatches = $false
if (Test-Path $AtomicSourceMarker) {
    try {
        $InstalledSource = Get-Content $AtomicSourceMarker -Raw | ConvertFrom-Json
        $SourceMatches = (
            [string]$InstalledSource.owner -eq $AtomicRepoOwner -and
            [string]$InstalledSource.repo -eq $AtomicRepoName -and
            [string]$InstalledSource.branch -eq $AtomicBranch
        )
    } catch {}
}
...
if (
    ...
    $NeedsDefinitions = (@($TechniqueFolders).Count -eq 0) -or (-not $SourceMatches)
    if (-not $SourceMatches -and @($TechniqueFolders).Count -gt 0) {
        Write-WarnFancy "Atomic source changed - refreshing definitions from configured fork"
    }
    ...
    if ($NeedsModule) {
        ...
        try {
            ...
            if (-not $NuGet) {
                ...
                if (-not [string]::IsNullOrWhiteSpace($Proxy)) { Write-Info "Transport" "EXPLICIT PROXY" }
                $NuGetArgs = @{
                    Name = "NuGet"
                    Scope = "CurrentUser"
                    Force = $true
    ...
    if ($Force -or $NeedsDefinitions) {
        if (
            -not (Test-InternetName -Name "github.com") `
            -and [string]::IsNullOrWhiteSpace($Proxy)
        ) {
            throw "Atomic definitions are missing and github.com cannot be resolved."
        }

        New-Item -ItemType Directory -Path $AtomicBase -Force | Out-Null

        $DownloadRoot = Join-Path $AtomicBase "_purplepoc_download"
        $CloneRoot = Join-Path $DownloadRoot "repo"
        $ArchiveFile = Join-Path $DownloadRoot "atomic-red-team.zip"
        Remove-Item $DownloadRoot -Recurse -Force -ErrorAction SilentlyContinue
        New-Item -ItemType Directory -Path $DownloadRoot -Force | Out-Null

        $DownloadedAtomics = $null
        $Git = Get-Command git.exe -ErrorAction SilentlyContinue
        if (-not $Git) { $Git = Get-Command git -ErrorAction SilentlyContinue }

        if ($Git) {
            Write-Step "Downloading Atomic Red Team from Jan's GitHub fork (git sparse clone)"
            Write-Info "Repository" $AtomicRepoGit
            $GitArgs = @()
            if (-not [string]::IsNullOrWhiteSpace($Proxy)) {
                $GitArgs += @("-c", "http.proxy=$Proxy")
            }
            $GitArgs += @("clone","--depth","1","--filter=blob:none","--sparse","--branch",$AtomicBranch,$AtomicRepoGit,$CloneRoot)
            & $Git.Source @GitArgs

            if ($LASTEXITCODE -eq 0) {
                $SparseArgs = @("-C",$CloneRoot,"sparse-checkout","set","atomics")
                & $Git.Source @SparseArgs
                if ($LASTEXITCODE -eq 0 -and (Test-Path (Join-Path $CloneRoot "atomics"))) {
                    $DownloadedAtomics = Join-Path $CloneRoot "atomics"
                    Write-Pass "Atomic definitions downloaded from configured fork via git"
                }
            }
        }

        if (-not $DownloadedAtomics) {
            Write-WarnFancy "Git unavailable/failed - using proxy-aware GitHub ZIP fallback"
            Write-Info "Network source" $AtomicArchiveUrl

            $AtomicDownloadArgs = @{
                Uri = $AtomicArchiveUrl
                OutFile = $ArchiveFile
                UseBasicParsing = $true
                TimeoutSec = 180
                ErrorAction = "Stop"
            ...
            if (-not [string]::IsNullOrWhiteSpace($Proxy)) {
                $AtomicDownloadArgs["Proxy"] = $Proxy
            }

            Invoke-WebRequest @AtomicDownloadArgs
            Expand-Archive -LiteralPath $ArchiveFile -DestinationPath $DownloadRoot -Force

            $ExtractedRepository = Get-ChildItem -LiteralPath $DownloadRoot -Directory |
                Where-Object { $_.Name -ne "repo" } |
                Select-Object -First 1

            if ($ExtractedRepository) {
                $DownloadedAtomics = Join-Path $ExtractedRepository.FullName "atomics"
            }
        }

        if (-not $DownloadedAtomics -or -not (Test-Path $DownloadedAtomics)) {
            throw "Configured Atomic fork did not provide an atomics folder."
        }

        if (Test-Path $AtomicDefinitions) {
            Remove-Item $AtomicDefinitions -Recurse -Force -ErrorAction Stop
        }
        Copy-Item -LiteralPath $DownloadedAtomics -Destination $AtomicDefinitions -Recurse -Force

        # The fork still contains upstream redcanaryco URLs inside Atomic YAML.
        # Redirect only atomic-red-team repository references to Jan's fork.
        $RewrittenFiles = 0
        Get-ChildItem -LiteralPath $AtomicDefinitions -Recurse -File |
            Where-Object { $_.Extension -in @(".yaml",".yml",".md",".ps1",".cmd",".bat",".sh") } |
            ForEach-Object {
                $Original = Get-Content -LiteralPath $_.FullName -Raw -ErrorAction SilentlyContinue
                if ($null -eq $Original) { return }

                $Updated = $Original.Replace(
                    "redcanaryco/atomic-red-team",
                    "$AtomicRepoOwner/$AtomicRepoName"
                )
        ...
        $RemainingUpstream = @(
            Get-ChildItem -LiteralPath $AtomicDefinitions -Recurse -File -Filter *.yaml |
            Select-String -SimpleMatch "redcanaryco/atomic-red-team" -ErrorAction SilentlyContinue
        )

        if ($RemainingUpstream.Count -gt 0) {
            throw "Atomic rewrite incomplete: upstream redcanaryco references remain in YAML."
        }
        ...
        } | ConvertTo-Json | Set-Content -Path $AtomicSourceMarker -Encoding UTF8

        Remove-Item $DownloadRoot -Recurse -Force -ErrorAction SilentlyContinue

        Write-Pass ("Atomic definitions READY from github.com/{0}/{1}; rewritten files: {2}" -f $AtomicRepoOwner,$AtomicRepoName,$RewrittenFiles)
...
$Status = @{
    ...
    atomic_repo_owner = $AtomicRepoOwner
    atomic_repo_name = $AtomicRepoName
    atomic_branch = $AtomicBranch
    atomic_repo_url = $AtomicRepoWeb
}
...
if (-not $Quiet -and -not [string]::IsNullOrWhiteSpace($Proxy)) { Write-Info "pip transport" "EXPLICIT --proxy" }
$CertipyArgs = @(
    "-m", "pip", "install",
...
if ([string]::IsNullOrWhiteSpace($AtomicRepoName)) { $AtomicRepoName = "atomic-red-team" }
if ([string]::IsNullOrWhiteSpace($AtomicBranch)) { $AtomicBranch = "master" }
Write-Info "Atomic source" ("github.com/{0}/{1} [{2}]" -f $AtomicRepoOwner,$AtomicRepoName,$AtomicBranch)
...
if ($RuntimeSelection) {
    ...
    if ($DependencyState.Ready) {
        ...
        if ($Python -eq $ProjectPython -and -not $DependencyState.Ready) {
            ...
            if (-not [string]::IsNullOrWhiteSpace($Proxy)) { Write-Info "pip transport" "EXPLICIT --proxy" }
...
if (Test-Path $AtomicSetup) {
    try {
        $AtomicSetupArgs = @{
            AtomicRepoOwner = $AtomicRepoOwner
            AtomicRepoName = $AtomicRepoName
            AtomicBranch = $AtomicBranch
        }
        if (-not [string]::IsNullOrWhiteSpace($Proxy)) {
            $AtomicSetupArgs["Proxy"] = $Proxy
        }
        & $AtomicSetup @AtomicSetupArgs
...
function Show-PurplePOCHelp {
    Write-Host ""
    Write-Host "PurplePOC 1.0.14 - Selective MITRE Execution" -ForegroundColor Cyan
    Write-Host ""
...
PurplePOC 1.0.14 adds AUTO coverage for Windows event-log manipulation and
...
PurplePOC 1.0.14 expands the AUTO/Atomic scenario substantially:
...
PurplePOC 1.0.14 fixes a false-negative smoke test introduced in 1.9.4.
...
PurplePOC 1.0.14 fixes scenario startup exiting with code 2 before T-001.
...
PurplePOC 1.0.14 removes stale smoke-test assertions left behind by the
...
The smoke test now validates the actual 1.0.14 parser implementation and keeps
...
PurplePOC 1.0.14 fixes the generated `rich_app.py` indentation error in the
...
PurplePOC 1.0.14 fixes the Rich UI crash caused by referencing `args.guided`
...
PurplePOC 1.0.14 is unattended by default: AUTO/Atomic tests run normally,
...
PurplePOC 1.0.14 replaces the large raw-output area in the live Rich UI with a
...
PurplePOC 1.0.14 fixes the dashboard appearing idle while scenario work was
...
PurplePOC 1.0.14 changes the Rich Live renderer for Windows PowerShell/conhost.
...
PurplePOC 1.0.14 fixes the behavior where the Rich UI disappeared as soon as
...
PurplePOC 1.0.14 fixes the Rich UI abort with exit code `5`.
...
PurplePOC 1.0.14 fixes the actual generator bug behind the missing
`Setup-AtomicRedTeam.ps1`.
...
PurplePOC 1.0.14 synchronizes the displayed version with the generated
...
PurplePOC 1.0.14 uses a real Python Rich terminal application for the elevated
...
PurplePOC 1.0.14 replaces the old verbose scenario output with a compact
structured renderer.
...
PurplePOC 1.0.14 gives the elevated PowerShell child process the same branded
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\PurplePOC-1.0.14-execution-bypass-header.ps1
```
...
## Interactive MITRE dashboard (v1.0.14)
...
## Complete proxy routing (v1.0.14)

v1.0.14 closes two proxy gaps from v1.0.12:

1. Atomic Red Team definitions are no longer downloaded through the upstream
   `Install-AtomicsFolder` helper. That helper performs an internal
   `Invoke-WebRequest` without exposing a proxy parameter. PurplePOC now
   downloads the official Atomic Red Team GitHub ZIP itself using an explicit
   `-Proxy` argument and then extracts the `atomics` folder locally.

2. `Invoke-AtomicTest -GetPrereqs` runs in separate PowerShell child processes.
   PurplePOC now injects the configured proxy into every Atomic prereq,
   execution and cleanup child:
   `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, `PIP_PROXY`,
   `WebRequest.DefaultWebProxy`, default `Invoke-WebRequest` /
   `Invoke-RestMethod` proxy values, and BITS cmdlet proxy defaults.

Verify the actual bootstrap endpoints before a run:

```powershell
.\Test-PurplePOCProxy.ps1
```

or:

```powershell
.\Test-PurplePOCProxy.ps1 -Proxy http://proxy.company.local:8080
```

The diagnostic checks PyPI, PowerShell Gallery, GitHub Raw and the Atomic Red
Team GitHub repository using an explicit PowerShell `-Proxy`.

Note: PurplePOC cannot force an arbitrary third-party executable to use a proxy
if that executable deliberately ignores both proxy environment variables and
Windows/PowerShell proxy configuration. Common PowerShell web cmdlets, pip,
requests, curl/git-style tools and PowerShell BITS cmdlet usage are covered.


## Atomic fork source (v1.0.14)

Default source: `https://github.com/jan85f/atomic-red-team`, branch `master`.

PurplePOC stores the source in `data\atomic-source.json`. It prefers a shallow
Git sparse clone of only `atomics` and uses the configured proxy for Git.
If Git is unavailable or fails, it falls back to an explicit proxy-aware ZIP
download from the same fork.

Existing Atomic definitions are refreshed once when their source marker does
not match the configured fork.

Atomic YAML files can still contain payload/GetPrereqs URLs pointing to
`redcanaryco/atomic-red-team`. PurplePOC rewrites that repository namespace to
the configured fork after download and verifies that no such upstream
references remain in YAML. Unrelated third-party URLs are not rewritten.
